...
def samples_per_unit(note, sample_rate):
  frequency = get_frequency(note)
  sampling_atom = 1/frequency if frequency > 0 else 0
  return int(round(sampling_atom*sample_rate*get_duration(note)))

def sample(note, t):
  return math.sin(2 * math.pi * get_frequency(note) * t) * get_volume(note)

def sample_audio(note, volume, sample_rate):
  n_samples = samples_per_unit(note, sample_rate)
...
def build_track_data(track, volume, sample_rate):
  data = numpy.empty(0, dtype = numpy.int16)
  for i in range(len(track)):
    next_data = sample_audio(track[i], volume, sample_rate)
    data = numpy.concatenate([data, next_data])
  return data

def build_channel_data(tracks, volume, sample_rate):
  data = numpy.empty(0, dtype = numpy.int16)
  track_data = []
  adjusted_volume = volume - len(tracks)
  for track in tracks:
    track_data.append(
      build_track_data(track, adjusted_volume, sample_rate)
    )
  # merge tracks
...
def build_wav(
  ...
  if tracks and left_tracks == None and right_tracks == None:
    data = build_channel_data(tracks, volume, sample_rate)
    ...
    l_data = build_channel_data(left_tracks, volume, sample_rate)
    r_data = build_channel_data(right_tracks, volume, sample_rate)
...
def test1():
  track1 = [100]*300
  track2 = [101]*300
  build_wav("wave_gen_tests/test1.wav", [track1, track2])


def test2():
  track1 = [100]*300
  track2 = [200]*300
  build_wav("wave_gen_tests/test2.wav", left_tracks=[track1], right_tracks=[track2])

def test3():
  track1 = [100]*30
  track2 = [Note(200, 0.5)]*300
  build_wav("wave_gen_tests/test3.wav", [track1, track2])

def test4():
  track1 = [Note(100)]*300
  track2 = [Note(200, 0.1)]*300
  build_wav("wave_gen_tests/test4.wav", left_tracks=[track1], right_tracks=[track2])
